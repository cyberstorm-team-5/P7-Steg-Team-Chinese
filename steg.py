...
#              The Python code will implement both the bit and byte steg algorithms
#              for storing and retrieving data. The sentiel value will be defined at
#              the top (default of 6 bytes) and appended left to right at the end of
#              the hidden data. The general usage for the code is :
#              ./steg.py -(bB) -(sr) -o<val> [-i<val>] -w<val> [-h<val>]
################################################################################
...
def storeData(wrapper, offset, interval):
	...
	if(sys.argv[1] == "-B"):
		...
		#bit method:
                ...
                for i in range(len(hidden)):
                        for j in range(8):
                                wrapper[offset] = chr(ord(wrapper[offset]) & 11111110)
                                wrapper[offset] = chr(ord(wrapper[offset]) | ((ord(hidden[i]) & 10000000) >> 7))
                                hidden[i] = chr(ord(hidden[i]) >> 1)
                                offset += interval
